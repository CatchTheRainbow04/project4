...
class ProductController extends Controller
{
    ...
    {
        $products = Product::with(['images', 'tags', 'user', 'category'])->get();
        $products->transform(function ($product) {
            $product->feature_image_url = $product->feature_image_path ? Storage::url($product->feature_image_path) : null;
            if ($product->images) {
                $product->images->transform(function ($img) {
                    $img->image_url = $img->image_path ? Storage::url($img->image_path) : null;
                    return $img;
                });
            }
            return $product;
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $featureImagePath = $featureImage->storeAs('products/feature', $featureImageName, 'public');
    ...
    {
        $product = Product::with(['images', 'tags', 'user', 'category'])->findOrFail($id);
        $product->feature_image_url = $product->feature_image_path ? Storage::url($product->feature_image_path) : null;
        if ($product->images) {
            $product->images->transform(function ($img) {
                $img->image_url = $img->image_path ? Storage::url($img->image_path) : null;
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->hasFile('feature_image_path')) {
                ...
                if ($product->feature_image_path && Storage::disk('public')->exists($product->feature_image_path)) {
                    Storage::disk('public')->delete($product->feature_image_path);
                }
                $featureImage = $request->file('feature_image_path');
                $featureImageName = time() . '_' . $featureImage->getClientOriginalName();
                $featureImagePath = $featureImage->storeAs('products/feature', $featureImageName, 'public');
