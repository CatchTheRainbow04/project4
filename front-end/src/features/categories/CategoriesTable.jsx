...
function CategoriesTable({ onEditCategory }) {
  ...
  const getRowClassName = (category) => {
    ...
    return `cursor-pointer transition ${isEvenId ? "bg-gray-50 dark:bg-gray-700/30" : "bg-white dark:bg-gray-800"} hover:bg-blue-50 dark:hover:bg-blue-900/20`;
  };
  ...
  return (
    <div className="col-span-full xl:col-span-8 bg-white dark:bg-gray-800 shadow-xs rounded-xl">
      <header className="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60 flex justify-between items-center">
        <h2 className="font-semibold text-gray-800 dark:text-gray-100">Danh sách danh mục</h2>
        <div className="relative w-64">
          ...
          />
          <svg className="w-5 h-5 absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <AnimatePresence>
            ...
            >
              <thead className="text-xs uppercase text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-700/50 rounded-xs">
                <tr>
                  <th className="p-2 sticky top-0 bg-gray-50 dark:bg-gray-700">
                    <div className="font-semibold text-left z-100">Tên danh mục</div>
                  </th>
                </tr>
              </thead>
              <tbody className="text-sm font-medium divide-y divide-gray-100 dark:divide-gray-700/60">
                {loading ? (
                  <tr>
                    <td colSpan={3} className="text-center py-8 text-blue-400">Đang tải...</td>
                  </tr>
                  ...
                  filteredCategories.map(cat => (
                    ...
                    >
                      <td className={`p-2 ${cat.parent_id === null ? 'font-bold' : ''}`}>{cat.name}</td>                   
                    </motion.tr>
                  ))
                ) : (
                  <motion.tr
                    ...
                    animate={{ opacity: 1 }}
                  >
                    <td colSpan={3} className="text-center py-8 text-blue-400">Không tìm thấy danh mục</td>
                  </motion.tr>
                )}
              </tbody>
            </motion.table>
